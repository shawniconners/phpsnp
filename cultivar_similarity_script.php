<?php
	...
	$objRequest->cultivar_count = filter_input(INPUT_GET, "cultivar_count", FILTER_VALIDATE_INT);
	//****************************************************************************************************************
	...
	$objAssembly->study->cultivar_count = $objRequest->cultivar_count;
	//****************************************************************************************************************
	...
	for($intLoopCounter = $objAssembly->study->cultivar_key+1; $intLoopCounter < $objAssembly->study->cultivar_count; $intLoopCounter++){
		...
		$objAssembly->sql = "SELECT JSON_EXTRACT(results, '$[".$objAssembly->study->cultivar_key."]') AS source_cultivar, JSON_EXTRACT(results, '$[".$intLoopCounter."]') AS compare_cultivar FROM tblStudy".$objAssembly->study->id."Structure".$objAssembly->structure->id."SNPs WHERE position >= :start_position AND position <= :stop_position;";
		...
		//echo "Mismatch between ".$objAssembly->study->cultivar_key." to ".$intLoopCounter.": ".count($objAssembly->study->mismatch_alleles)." of ".count($objAssembly->study->snps)."<br />";
	}
	//****************************************************************************************************************
