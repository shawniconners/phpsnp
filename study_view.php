<?php
	...
	$objStudy->sql = "SELECT assembly_id, name, source, snp_count, cultivar_count, structures FROM tblStudies WHERE id = :id;";
	...
	//****************************************************************************************************************
	$objStudy->sql = "SELECT name FROM tblAssemblies WHERE id = :id;";
	$objStudy->prepare = $objSettings->database->connection->prepare($objStudy->sql);
...
//echo "<pre>";
//print_r($objStudy->assembly->structures);
//echo "</pre>";
//exit;
?>
<!doctype html>
					...
					<p class="lead">
						...
						<?php
							...
							if($objStudy->error_count > 0){
								...
								//****************************************************************************************************************
								echo "<a id='elmStudyViewErrors' class='btn btn-warning' href='study_errors_script.php?id=".$objStudy->id."' role='button'>Download Errors <span class='badge badge-light'>".$objStudy->error_count."</span></a>";
								//****************************************************************************************************************
							...
							<?php
								...
								foreach ($objStudy->assembly->structures as $arrStructure) {
									if($arrStructure["snp_count"] > 0){
										...
										//****************************************************************************************************************
										echo "<tr><td><a href='snp_list.php?study_id=".$objStudy->id."&structure_id=".$arrStructure["id"]."'>".$arrStructure["name"]."</a></td><td>".number_format($arrStructure["sequence_length"])."</td><td>".number_format($arrStructure["snp_count"])."</td></tr>";
										//****************************************************************************************************************
